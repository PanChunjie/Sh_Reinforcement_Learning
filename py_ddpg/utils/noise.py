# original code from： https://github.com/hill-a/stable-baselines/blob/master/stable_baselines/ddpg/noise.py
import numpy as np
...
class OrnsteinUhlenbeckActionNoise(object):
    ...
    def __init__(self, mean=0.5, sigma=0.1, theta=1.0, dt=1e-2, action_dim=1, initial_noise=None):
        self._theta = theta
        self._mu = mean
        self._sigma = sigma
        self._dt = dt
    ...
    def ou_noise(self):
        noise = self.noise_prev + self._theta * (self._mu - self.noise_prev) * self._dt + self._sigma * np.sqrt(self._dt) * np.random.normal(size=self.action_dim)
    ...
    def reset(self):
        ...
        """
        self.noise_prev = self.initial_noise if self.initial_noise is not None else np.zeros_like(np.zeros(self.action_dim))
